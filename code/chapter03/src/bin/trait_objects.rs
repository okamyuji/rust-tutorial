...
fn practical_trait_objects() {
    ...
    impl PluginManager {
        ...
        fn execute_all(&self, input: &str) -> Vec<(String, String)> {
            self.plugins
                .iter()
                .map(|plugin| {
                    (plugin.name().to_string(), plugin.execute(input))
                })
                .collect()
        }
    }
    ...
    let processors: Vec<Box<dyn Processor>> = vec![
        Box::new(CompressProcessor),
        Box::new(EncryptProcessor),
    ];
