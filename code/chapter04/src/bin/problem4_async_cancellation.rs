...
impl CancellableTask {
    ...
    pub async fn compute_intensive_task(&self, iterations: u64) -> Result<u64, &'static str> {
        ...
        for i in 0..iterations {
            // 定期的にキャンセレーションをチェック
            if i % 1000 == 0 && self.token.is_cancelled() {
...
impl TimeoutHandler {
    ...
    pub async fn select_first<F1, F2, T1, T2>(future1: F1, future2: F2) -> Result<T1, T2>
    where
        F1: Future<Output = T1>,
        F2: Future<Output = T2>,
    {
...
impl Drop for ResourceManager {
    fn drop(&mut self) {
        if !self.cleanup_performed && !self.resources.is_empty() {
            println!(
                "  警告: ResourceManagerがdropされましたが、クリーンアップが実行されていません"
            );
...
async fn demo_timeout_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match TimeoutHandler::with_timeout(task.download_task(1000, 50), Duration::from_secs(3)).await {
        Ok(Ok(data)) => {
            println!(
                "  ダウンロード完了: {} バイト, 経過時間: {:.2}秒",
                data.len(),
                start.elapsed().as_secs_f64()
            );
        }
...
mod tests {
    ...
    async fn test_timeout_handling() {
        ...
        // 非常に短いタイムアウト
        let result =
            TimeoutHandler::with_timeout(task.download_task(1000, 100), Duration::from_millis(1))
                .await;
