...
impl CancellableTimer {
    pub fn new(duration: Duration) -> (Self, CancelHandle) {
        ...
        let cancel_handle = CancelHandle { cancel_flag };
...
async fn precision_timer_test() {
    ...
    let timers = vec![
        ...
        (
            "高精度",
            PrecisionTimer::with_precision(Duration::from_millis(100), Duration::from_micros(500)),
        ),
        (
            "低精度",
            PrecisionTimer::with_precision(Duration::from_millis(100), Duration::from_millis(10)),
        ),
    ];
