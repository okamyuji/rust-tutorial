...
async fn stream_combinators() {
    ...
    let flattened: Vec<i32> = nested.flat_map(stream::iter).collect().await;
    ...
    let taken: Vec<i32> = stream::iter(1..=10).take(5).collect().await;
    println!("    最初の5個: {:?}", taken);

    let skipped: Vec<i32> = stream::iter(1..=10).skip(5).collect().await;
...
async fn custom_streams() {
    ...
    let result_stream = stream::iter(vec![Ok(1), Ok(2), Err(MyError), Ok(3)]);

    // try_collectでエラーハンドリング
    let collected: Result<Vec<i32>, MyError> = result_stream.try_collect().await;
    ...
    // エラーをフィルタリング
    let result_stream2 = stream::iter(vec![Ok(1), Ok(2), Err(MyError), Ok(3)]);
...
async fn stream_processing_patterns() {
    ...
    let chunked: Vec<Vec<i32>> = stream::iter(1..=10).chunks(3).collect().await;
    ...
    let processed = event_stream.scan(0, |state, event| match event {
        Event::Data(data) => {
            *state += 1;
        ...
        Event::Complete => futures::future::ready(None),
    });
