...
fn shared_config_example() {
    ...
    impl AppConfig {
        fn new() -> Self {
            ...
            AppConfig { settings, feature_flags }
        }
    ...
    println!("\n最終設定:");
    println!("  timeout: {}", final_config.get_setting("timeout").unwrap_or_default());
    println!("  experimental_feature: {}", final_config.is_feature_enabled("experimental_feature"));
}
...
fn tree_structure_example() {
    ...
    impl FileNode {
        ...
        fn print_filesystem(&self, indent: usize) {
            ...
            if self.is_file {
                println!("{}📄 {} ({} bytes)", prefix, self.name, self.size.unwrap_or(0));
            } else {
